...
## 0. Create linux instance and install k3s
To create multipass instance:
```
multipass launch -n workshop -d 8G -m 4G
multipass shell workshop
```

To install k3s on Linux:  
```
...
And to ensure you have the export each time you open the shell again:
```
...
* Create a file **k3s_values.yaml** and add the following contents, replacing the [HOSTNAME]:
  ```
  monitors:
